...
contract MultiSignPaymentWallet {
    ...
    mapping(uint256 => ApprovalInfo[]) public approvalsHistory;
    ...
    constructor(address[] memory _owners, uint _requiredApprovals, address[] memory _payees, uint256[] memory _shares) {
        _status = 1;
    ...
    function approveTransaction(uint txId) external onlyOwner {
        ...
        approvalsHistory[txId].push(ApprovalInfo({
            approver: msg.sender,
    ...
    function getTransactions() external view returns (Transaction[] memory) {
        Transaction[] memory all = new Transaction[](nextTransactionId);
        for (uint i = 0; i < nextTransactionId; i++) {
            all[i] = transactions[i];
        }
        return all;
    }
    ...
    function getApprovalsHistory(uint txId) external view returns (uint256 transactionId, uint256 totalApprovals, ApprovalInfo[] memory approvalsSuccesed) {
        Transaction storage transaction = transactions[txId];
        return (txId, transaction.approvalCount, approvalsHistory[txId]);
    }
}